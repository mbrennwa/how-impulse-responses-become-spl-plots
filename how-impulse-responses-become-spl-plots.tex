...
\section{From a real-world impulse response to the SPL curve: time gating (reality bites!)}\secref{from_IR_to_SPL}
...
The relationships described in \secn{Fourier_Theory} define the fundamental rules on which the Fourier transform operates. However, real-world measurements have a tendency to not play nice with these rules, which is why techniques to reduce frequency leakage or to improve the visual appearance of and SPL curve are applied -- sometimes for the better, sometimes not.
