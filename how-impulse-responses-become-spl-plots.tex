...
\figr{FIGURE3} shows the implied periodic signal underlying the Fourier transform (see \secn{Fourier_periodic}). There is a small discontinuity at the transitions between the cycles. This sharp step in the periodic signal introduces spurious high-frequency content in the Fourier spectrum, which does not exist in the sound radiated by the speaker. 
DISCUSS WINDOW FUNCTIONS TO FORCE PERIODIC SIGNAL. attenuates/messes the tail of the impulse response, so data is changed or lost.
